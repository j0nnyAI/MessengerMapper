...
RELEASE_STRIP_SCRIPT: Path | None = None
...
def _engine_scripts_from_project_root(project_root: Path) -> Path | None:
    """Sibling/embedded THE-NEXUS/scripts/ next to *project_root*, mirroring the
    structural check nexus_paths.resolve_nexus_dir() uses once it's importable."""
    root = project_root.expanduser().resolve()
    for candidate in (root / "THE-NEXUS", root.parent / "THE-NEXUS"):
        module_file = candidate / "scripts" / "nexus_paths.py"
        if module_file.is_file():
            return module_file
    return None
...
def bootstrap_engine_import(
    *,
    ...
    # FAB-14: the hook passes --project-root to this script precisely so a fresh
    # tenant commit (no NEXUS_PATH env var, no inherited sys.path) can still locate
    # the engine. Without this, every tenant's first real commit silently no-ops.
    if project_root_override is not None:
        module_file = _engine_scripts_from_project_root(project_root_override)
        if module_file is not None:
            _import_nexus_paths_module(module_file)
            return

    module_file = _engine_scripts_from_project_root(_SCRIPT_DIR.parent)
    if module_file is not None:
        _import_nexus_paths_module(module_file)
        return
    ...
    raise ImportError(
        ...
        "PYTHONPATH to include THE-NEXUS/scripts, or pass --project-root / "
        "--vault-root pointing at a tree with a THE-NEXUS sibling."
    )
...
def init_topology(
    *,
    ...
    RELEASE_STRIP_SCRIPT = nexus_dir / "scripts" / "release-strip.py"
    return True
...
def dispatch_release_strip() -> None:
    """Spawn release-strip.py in background if manifest exists in vault."""
    if PROJECT_ROOT is None or VAULT_DIR is None or RELEASE_STRIP_SCRIPT is None:
        return
    manifest = VAULT_DIR / "10-DEPLOYMENT" / "nexus-strip-manifest.yml"
    if not manifest.is_file():
        return  # feature not enabled for this project
    script = _ensure_bounded(RELEASE_STRIP_SCRIPT)
