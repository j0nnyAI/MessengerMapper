...
2. After the handoff doc is written, run TEARDOWN cascade:
   ...
   - STEP 1: `python ../THE-NEXUS/scripts/handoff.py --project-root . --vault-root ../MessengerMapper-VAULT -write <shortname>` (pick a kebab-case shortname ≤4 words describing the session)
   - STEP 2: `python ../THE-NEXUS/scripts/review-commit.py --project-root . --vault-root ../MessengerMapper-VAULT --sha PENDING --scope reviewer --dry-run` (pending diff, not last commit)
